...
gulp.task('watch', gulp.series('sass', 'scripts', 'browser-sync', function () {
    ...
    gulp.watch('libs/**/*.js', 'app/js/common.js', gulp.series('scripts'));
...
gulp.task('build', gulp.series('remove', 'imagemin', 'sass', 'scripts', function () {
...
gulp.task('remove', gulp.series(() => {
    del.sync('dist')
}));
gulp.task('clearance', gulp.series(() => {
    cache.clearAll()
}));
